...
");
$listEvaluasi = [];
while($ev = mysqli_fetch_assoc($listEvaluasiRes)){
    $listEvaluasi[] = $ev;
}

// proses tambah riwayat evaluasi
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['mode'] ?? '') === 'insert') {
    $id            = $_POST['id'] ?? '';
    $kode_evaluasi = $_POST['kode_evaluasi'] ?? '';
    $tahun         = $_POST['tahun'] ?? '';
    $bulan         = $_POST['bulan'] ?? '';
    $keterangan    = $_POST['keterangan'] ?? '';

    // langsung insert tanpa cek file
    $stmt = $conn->prepare("INSERT INTO evaluasi_kinerja_pegawai
        (id, kode_evaluasi, tahun, bulan, keterangan)
        VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("sssss", $id, $kode_evaluasi, $tahun, $bulan, $keterangan);
    $stmt->execute();
    $stmt->close();
}
...
if($filter===''){
    $sql = "SELECT p.id AS peg_id, p.nik, p.nama, d.nama AS departemen, p.bidang,
                   r.tahun, r.bulan, r.kode_evaluasi, r.keterangan,
                   e.nama_evaluasi
            FROM pegawai p
            LEFT JOIN departemen d ON p.departemen = d.dep_id
            LEFT JOIN evaluasi_kinerja_pegawai r ON r.id = p.id
...
while($row=mysqli_fetch_assoc($result)){
    ...
    if($row['kode_evaluasi']!==null){
        $pegawaiData[$pegId]['riwayat'][] = [
            'kode_evaluasi'=>$row['kode_evaluasi'],
            'nama_evaluasi' => $row['nama_evaluasi'],
            'tahun'=>$row['tahun'],
            'bulan'=>$row['bulan'],
...
<head>
  <meta charset="UTF-8">
  <title>Riwayat Evaluasi Pegawai</title>
...
<main class="main-content container-fluid mt-4">
  <div class="card shadow">
    <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
      <h5 class="mb-0 text-uppercase flex-grow-1 text-center">Riwayat Evaluasi Pegawai</h5>
        ...
        <table class="table table-striped table-bordered table-riwayat_evaluasi align-middle">
          <thead class="table-dark text-center">
            <tr>
              ...
              <th>Nama</th>
              <th>Depart</th>
              <th>Riwayat Evaluasi Pegawai</th>
              <th>Aksi</th>
            </tr>
          </thead>
          <tbody>
            ...
            foreach($pegawaiData as $pegId=>$data): ?>
              <tr>
                      ...
                      <tbody>
                        <?php $no=1; foreach($data['riwayat'] as $rj): ?>
                          <tr>
                            ...
                            <td class="text-success"><?= htmlspecialchars($rj['nama_evaluasi']) ?></td>
                ...
                <td class="text-center">
                  <a href="detail_riwayat_evaluasi.php?id=<?= $pegId ?>" class="btn btn-info btn-sm">Detail</a>
                </td>
              </tr>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header bg-success text-white">
        <h5 class="modal-title">Tambah Riwayat Evaluasi</h5>
      ...
      <form method="post" enctype="multipart/form-data" action="">
        ...
        <div class="modal-body">
          ...
          <div class="mb-3">
            <label class="form-label">Evaluasi Kinerja</label>
            <select name="kode_evaluasi" class="form-select">
              <option value="">-- Pilih Evaluasi --</option>
              <?php foreach($listEvaluasi as $ev): ?>
                <option value="<?= htmlspecialchars($ev['kode_evaluasi']) ?>">
                  <?= htmlspecialchars($ev['kode_evaluasi']) ?> - <?= htmlspecialchars($ev['nama_evaluasi']) ?>
                </option>
              <?php endforeach; ?>
            </select>
          </div>
