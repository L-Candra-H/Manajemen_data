...
if (!cekAkses('pegawai_admin') && !cekAkses('pegawai_user')) {
    echo "<div class='alert alert-danger'>Akses ditolak. Anda tidak memiliki hak ke menu Master Tunjangan Harian.</div>";
    exit;
}
...
// hitung total data
$totalResult = mysqli_query($conn, "SELECT COUNT(*) AS total FROM master_tunjangan_harian");
...
$sql = "SELECT id, nama, tnj FROM master_tunjangan_harian ORDER BY nama LIMIT $limit OFFSET $offset";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['mode']) && $_POST['mode'] === 'insert') {
    ...
    if ($nama !== null && $tnj !== null) {
        $stmt = $conn->prepare("INSERT INTO master_tunjangan_harian (nama, tnj) VALUES (?, ?)");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['mode']) && $_POST['mode'] === 'edit') {
    ...
    if ($id !== null && $nama !== null && $tnj !== null) {
        $stmt = $conn->prepare("UPDATE master_tunjangan_harian SET nama=?, tnj=? WHERE id=?");
...
if (isset($_GET['hapus'])) {
    $id = $_GET['hapus'];
    $stmt = $conn->prepare("DELETE FROM master_tunjangan_harian WHERE id=?");
    $stmt->bind_param("i", $id);
    $stmt->execute();
    header("Location: master_tunjangan_harian.php");
    exit;
}

// Ambil data
$sql = "SELECT id, nama, tnj FROM master_tunjangan_harian ORDER BY id ASC LIMIT $limit OFFSET $offset";
$result = mysqli_query($conn, $sql);
?>
<!DOCTYPE html>
<html lang="id">
<head>
  <meta charset="UTF-8">
  <title>Master Tunjangan Harian</title>
  ...
  <main class="container-fluid mt-4">
    <div class="card shadow">
      <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
        <h5 class="mb-0 text-uppercase flex-grow-1 text-center">Master Tunjangan Harian</h5>
          ...
          <tbody>
            ...
            <tr>
              ...
              <td class="text-center">
                ...
                <a href="master_tunjangan_harian.php?hapus=<?= $row['id'] ?>"
    ...
    <div class="modal-dialog">
      <form method="post" class="modal-content">
        ...
        <div class="modal-header bg-success text-white">
          <h5 class="modal-title">Tambah Tunjangan Harian</h5>
    ...
    <div class="modal-dialog">
      <form method="post" class="modal-content">
        ...
        <div class="modal-header bg-warning text-dark">
          <h5 class="modal-title">Edit Tunjangan Harian</h5>
