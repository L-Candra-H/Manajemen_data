...
if (!cekAkses('pegawai_admin') && !cekAkses('pegawai_user')) {
    echo "<div class='alert alert-danger'>Akses ditolak. Anda tidak memiliki hak ke menu Set Lembur HR.</div>";
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['mode'] ?? '') === 'insert') {
    $tnj = $_POST['tnj'] ?? null;
    if ($tnj) {
        $stmt = $conn->prepare("INSERT INTO set_lemburhr (tnj) VALUES (?)");
        $stmt->bind_param("s", $tnj);
        $stmt->execute();
    }
    header("Location: set_lemburhr.php");
    exit;
}
...
if (isset($_GET['delete_tnj'])) {
    ...
    $stmt = $conn->prepare("DELETE FROM set_lemburhr WHERE tnj=?");
...
// hitung total data
$totalResult = mysqli_query($conn, "SELECT COUNT(*) AS total FROM set_lemburhr");
...
$sql  = "SELECT tnj FROM set_lemburhr ORDER BY tnj LIMIT $limit OFFSET $offset";
  ...
  <main class="container-fluid mt-4">
    <div class="card shadow">
      <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
        <h5 class="mb-0 text-uppercase flex-grow-1 text-center">Set Lembur HR</h5>
          ...
          <tbody>
            ...
            <tr>
              ...
              <td class="text-center">
                <a href="set_lemburhr.php?delete_tnj=<?= urlencode($row['tnj']) ?>" 
