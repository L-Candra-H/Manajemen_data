...
if (!cekAkses('pegawai_admin') && !cekAkses('pegawai_user')) {
    echo "<div class='alert alert-danger'>Akses ditolak. Anda tidak memiliki hak ke menu Status Wajib Pajak.</div>";
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['mode']) && $_POST['mode'] === 'update') {
        $stmt = $conn->prepare("UPDATE stts_wp SET ktg=? WHERE stts=?");
        $stmt->bind_param("ss", $ktg, $stts);
        $stmt->execute();
    } elseif (isset($_POST['mode']) && $_POST['mode'] === 'insert') {
        $stmt = $conn->prepare("INSERT INTO stts_wp (stts, ktg) VALUES (?, ?)");
        $stmt->bind_param("ss", $stts, $ktg);
        $stmt->execute();
    }
}
...
// hitung total data
$totalResult = mysqli_query($conn, "SELECT COUNT(*) AS total FROM stts_wp");
...
<head>
  <meta charset="UTF-8">
  <title>Status Wajib Pajak</title>
  ...
  <main class="container mt-4">
    <div class="card shadow">
      <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
        <h5 class="mb-0 text-uppercase flex-grow-1 text-center">Status Wajib Pajak</h5>
    ...
    <div class="modal-dialog">
      <form action="" method="post" class="modal-content">
        ...
        <div class="modal-header bg-success text-white">
          <h5 class="modal-title">Tambah Status Wajib Pajak</h5>
    ...
    <div class="modal-dialog">
      <form action="" method="post" class="modal-content">
        ...
        <div class="modal-header bg-warning">
          <h5 class="modal-title">Edit Status Wajib Pajak</h5>
