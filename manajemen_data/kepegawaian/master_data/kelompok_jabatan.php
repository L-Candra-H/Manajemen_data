...
if (!cekAkses('pegawai_admin') && !cekAkses('pegawai_user')) {
    echo "<div class='alert alert-danger'>Akses ditolak. Anda tidak memiliki hak ke menu Kelompok Jabatan.</div>";
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $kode  = $_POST['kode_kelompok'] ?? null;
    $nama  = $_POST['nama_kelompok'] ?? null;
    $indek = (isset($_POST['indek']) && is_numeric($_POST['indek'])) ? (int)$_POST['indek'] : 0;

    if (isset($_POST['mode']) && $_POST['mode'] === 'update') {
        $stmt = $conn->prepare("UPDATE kelompok_jabatan SET nama_kelompok=?, indek=? WHERE kode_kelompok=?");
        $stmt->bind_param("sis", $nama, $indek, $kode);
        $stmt->execute();
    } elseif (isset($_POST['mode']) && $_POST['mode'] === 'insert') {
        $stmt = $conn->prepare("INSERT INTO kelompok_jabatan (kode_kelompok, nama_kelompok, indek) VALUES (?, ?, ?)");
...
// hitung total data
$totalResult = mysqli_query($conn, "SELECT COUNT(*) AS total FROM kelompok_jabatan");
  ...
  <main class="container-fluid mt-4">
    <div class="card shadow">
      <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
        <h5 class="mb-0 text-uppercase flex-grow-1 text-center">Kelompok Jabatan</h5>
    ...
    <div class="modal-dialog">
      <form action="" method="post" class="modal-content">
        ...
        <div class="modal-header bg-success text-white">
          <h5 class="modal-title">Tambah Kelompok Jabatan</h5>
          <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
        </div>
        <div class="modal-body">
          <div class="mb-3"><label class="form-label">Kode Kelompok</label><input type="text" name="kode_kelompok" class="form-control" required></div>
          <div class="mb-3"><label class="form-label">Nama Kelompok</label><input type="text" name="nama_kelompok" class="form-control" required></div>
    ...
    <div class="modal-dialog">
      <form action="" method="post" class="modal-content">
        ...
        <div class="modal-header bg-warning">
          <h5 class="modal-title">Edit Kelompok Jabatan</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
        </div>
        <div class="modal-body">
          <div class="mb-3"><label class="form-label">Kode Kelompok</label>
            <input type="text" name="kode_kelompok" id="editKode" class="form-control bg-danger text-dark fw-bold" readonly>
          </div>
          <div class="mb-3"><label class="form-label">Nama Kelompok</label>
