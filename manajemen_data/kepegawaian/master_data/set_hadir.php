...
if (!cekAkses('pegawai_admin') && !cekAkses('pegawai_user')) {
    echo "<div class='alert alert-danger'>Akses ditolak. Anda tidak memiliki hak ke menu Set Hadir.</div>";
    exit;
}
error_reporting(0);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['mode'] ?? '') === 'insert') {
    $tnj = $_POST['tnj'] ?? null;
    if ($tnj) {
        $stmt = $conn->prepare("INSERT INTO set_hadir (tnj) VALUES (?)");
        $stmt->bind_param("s", $tnj);
        $stmt->execute();
    }
    header("Location: set_hadir.php");
    exit;
}
...
if (isset($_GET['delete_tnj'])) {
    ...
    $stmt = $conn->prepare("DELETE FROM set_hadir WHERE tnj=?");
...
// hitung total data
$totalResult = mysqli_query($conn, "SELECT COUNT(*) AS total FROM set_hadir");
...
$sql  = "SELECT tnj FROM set_hadir ORDER BY tnj LIMIT $limit OFFSET $offset";
...
<head>
  <meta charset="UTF-8">
  <title>Set Tunjangan Hadir</title>
  ...
  <main class="container-fluid mt-4">
    <div class="card shadow">
      <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
        <h5 class="mb-0 text-uppercase flex-grow-1 text-center">Set Tunjangan Hadir</h5>
          ...
          <tbody>
            ...
            <tr>
              ...
              <td class="text-center">
                <a href="set_hadir.php?delete_tnj=<?= urlencode($row['tnj']) ?>" 
