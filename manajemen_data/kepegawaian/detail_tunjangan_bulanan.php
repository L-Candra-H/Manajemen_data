...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['tambah'])) {
    ...
    $stmt  = $conn->prepare("INSERT INTO pnm_tnj_bulanan (id, id_tnj) VALUES (?, ?)");
    $stmt->bind_param("ii", $pegawai['id'], $idTnj);
    $stmt->execute();
    $stmt->close();
    header("Location: detail_tunjangan_bulanan.php?nik=" . urlencode($nik) . "&nama=" . urlencode($nama));
    exit;
}
...
if (isset($_GET['hapus'])) {
    ...
    $stmt    = $conn->prepare("DELETE FROM pnm_tnj_bulanan WHERE id_tnj=? AND id=?");
...
// ambil daftar master tunjangan bulanan yang belum dipakai
$sqlMaster = "
    SELECT id, nama, COALESCE(tnj,0) AS besar_tunjangan
    FROM master_tunjangan_bulanan
    WHERE id NOT IN (
        SELECT id_tnj FROM pnm_tnj_bulanan WHERE id = '{$pegawai['id']}'
    )
    ORDER BY nama ASC
";
...
<head>
  <meta charset="UTF-8">
  <title>Detail Tunjangan Bulanan</title>
    ...
    <div class="card shadow">
      ...
      <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
        <h5 class="mb-0 text-uppercase flex-grow-1 text-center">
          Detail Tunjangan Bulanan | <?= htmlspecialchars($pegawai['nik']) ?> - <?= htmlspecialchars($pegawai['nama']) ?>
        </h5>
      ...
      <div class="card-body p-3">
        ...
        <form method="post" class="d-flex flex-wrap align-items-center mb-3">
          ...
          <select name="id_tnj" class="form-select me-3" style="width:auto;" required>
            <option value="">-- Pilih Tunjangan Bulanan --</option>
          ...
          <table class="table table-striped table-bordered align-middle">
            ...
            <tbody>
              <?php if ($jmlData > 0): ?>
                <?php while ($row = mysqli_fetch_assoc($resTunjangan)): ?>
                  <tr>
                    ...
                    <td class="text-center">
                      <a href="detail_tunjangan_bulanan.php?nik=<?= urlencode($nik) ?>&nama=<?= urlencode($nama) ?>&hapus=<?= $row['id'] ?>"
                ...
                <?php endwhile; ?>
              <?php else: ?>
                <tr><td colspan="3" class="text-center text-muted">Belum ada tunjangan bulanan</td></tr>
              <?php endif; ?>
            </tbody>
          </table>
        </div>
