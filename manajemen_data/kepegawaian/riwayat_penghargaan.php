...
// proses tambah riwayat penghargaan
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['mode'] ?? '') === 'insert') {
    ...
    $pejabat_pemberi  = $_POST['pejabat_pemberi'] ?? '';
    $filePath         = "";

    if (!empty($_FILES['berkas']['name'])) {
        $fileName   = basename($_FILES['berkas']['name']);
        $targetDir  = __DIR__ . "/../../webapps/penggajian/pages/riwayatpenghargaan/berkas/";
        ...
        if (in_array($ext, ['jpg','jpeg','png','gif','pdf'])) {
            if (move_uploaded_file($_FILES['berkas']['tmp_name'], $targetFile)) {
                $filePath = "pages/riwayatpenghargaan/berkas/" . $fileName;
            } else {
    ...
    if ($filePath !== "") {
        $stmt = $conn->prepare("INSERT INTO riwayat_penghargaan 
            (id, jenis, nama_penghargaan, tanggal, instansi, pejabat_pemberi, berkas) 
            VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("sssssss", $id, $jenis, $nama_penghargaan, $tanggal,
                          $instansi, $pejabat_pemberi, $filePath);
        $stmt->execute();
        $stmt->close();
    }
}
...
if($filter===''){
    $sql = "SELECT p.id AS peg_id, p.nik, p.nama,
                   r.nama_penghargaan, r.tanggal, r.instansi, r.pejabat_pemberi
            FROM pegawai p
            LEFT JOIN riwayat_penghargaan r ON r.id = p.id
            WHERE 1=0";
} elseif($filter==='ALL'){
    $sql = "SELECT p.id AS peg_id, p.nik, p.nama,
                   r.nama_penghargaan, r.tanggal, r.instansi, r.pejabat_pemberi
            ...
            ORDER BY p.nik ASC, r.nama_penghargaan ASC
...
<main class="main-content container-fluid mt-4">
  <div class="card shadow">
    <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
      <h5 class="mb-0 text-uppercase flex-grow-1 text-center">Riwayat Penghargaan</h5>
        ...
        <table class="table table-striped table-bordered table-riwayat_penghargaan align-middle">
          <thead class="table-dark text-center">
            <tr>
              <th>NIP</th>
              <th>Nama</th>
              <th>Riwayat Penghargaan Pegawai</th>
              <th>Aksi</th>
            </tr>
          </thead>
          <tbody>
            ...
            foreach($pegawaiData as $pegId=>$data): ?>
              <tr>
                    ...
                    <table class="table table-sm table-bordered mb-0">
                      ...
                      <tbody>
                        <?php $no=1; foreach($data['riwayat'] as $rj): ?>
                          <tr>
                            <td><?= $no++ ?></td>
                            <td><span class="badge bg-info"><?= htmlspecialchars($rj['nama_penghargaan']) ?></span></td>
                            <td class="text-primary"><?= htmlspecialchars($rj['tanggal']) ?></td>
                            <td class="text-success"><?= htmlspecialchars($rj['instansi']) ?></td>
                            <td class="text-success"><?= htmlspecialchars($rj['pejabat_pemberi']) ?></td>
                          </tr>
                        <?php endforeach; ?>
                      </tbody>
                    </table>
                ...
                <td class="text-center">
                  <a href="detail_riwayat_penghargaan.php?id=<?= $pegId ?>" class="btn btn-info btn-sm">Detail</a>
                </td>
              </tr>
