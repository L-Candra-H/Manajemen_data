...
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['hapus_id'])){
    ...
    $kode_pencapaian = mysqli_real_escape_string($conn, $_POST['hapus_kode_pencapaian']);
    $tahun           = mysqli_real_escape_string($conn, $_POST['hapus_tahun']);
    $keterangan      = mysqli_real_escape_string($conn, $_POST['hapus_keterangan']);

    $sqlDel = "DELETE FROM pencapaian_kinerja_pegawai 
               WHERE id='$id'
                 AND kode_pencapaian='$kode_pencapaian' 
                 AND tahun='$tahun' 
    ...
    if(mysqli_query($conn,$sqlDel)){
        $id = $_GET['id'] ?? '';
        header("Location: detail_riwayat_pencapaian.php?id=".$id);
        exit;
    } else {
...
// --- QUERY DATA RIWAYAT ---
$id = $_GET['id'] ?? '';
$sql = "SELECT r.*, p.nik, p.nama, e.nama_pencapaian 
        FROM pencapaian_kinerja_pegawai r 
...
<head>
  <meta charset="UTF-8">
  <title>Detail Riwayat Pencapaian</title>
  ...
  <div class="card shadow">
    <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
      <h5 class="mb-0 text-uppercase text-center flex-grow-1">
        Detail Riwayat Pencapaian - <?= htmlspecialchars($dataPegawai['nama'] ?? '') ?>
      </h5>
      <div class="d-flex gap-2">
        <a href="riwayat_pencapaian.php" class="btn btn-light btn-sm">⬅️ Kembali</a>
      </div>
    </div>
    <div class="card-body">

      <!-- Ringkasan Riwayat Pencapaian -->
        ...
        <table class="table table-striped table-bordered table-riwayat_pencapaian align-middle">
          ...
          <tbody>
            <?php if(empty($riwayat)): ?>
              <tr><td colspan="10" class="text-center text-muted">Tidak ada riwayat pencapaian</td></tr>
            <?php else: ?>
              <?php foreach($riwayat as $rj): ?>
                <tr>
                  <td><?= $rj['tahun'] ?></td>
                  <td><?= $rj['bulan'] ?></td>
                  <td><?= $rj['nama_pencapaian'] ?></td>
                  <td><?= $rj['keterangan'] ?></td>
                  <td class="text-center">
                    <form method="post" onsubmit="return confirm('Yakin hapus riwayat pencapaian ini?')">
                      <input type="hidden" name="hapus_id" value="<?= $rj['id'] ?>">
                      <input type="hidden" name="hapus_kode_pencapaian" value="<?= $rj['kode_pencapaian'] ?>">
