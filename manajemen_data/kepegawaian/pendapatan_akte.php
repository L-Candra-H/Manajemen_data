...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $pendapatan = $_POST['pendapatan_akte'] ?? null;
    ...
    if ($_POST['mode'] === 'update') {
        $stmt = $conn->prepare("UPDATE set_akte SET pendapatan_akte=?, persen_rs=?, bagian_rs=?, persen_kry=?, bagian_kry=? WHERE tahun=? AND bulan=?");
        ...
        $cek = $conn->prepare("SELECT COUNT(*) FROM set_akte WHERE tahun=? AND bulan=?");
        ...
        if ($ada > 0) {
            ...
            $stmt = $conn->prepare("INSERT INTO set_akte (tahun, bulan, pendapatan_akte, persen_rs, bagian_rs, persen_kry, bagian_kry) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("iiiiiii", $tahun, $bulan, $pendapatan, $persen_rs, $bagian_rs, $persen_kry, $bagian_kry);
            $stmt->execute();
        }

    } elseif ($_POST['mode'] === 'delete') {
        $stmt = $conn->prepare("DELETE FROM set_akte WHERE tahun=? AND bulan=?");
...
// hitung total data
$totalResult = mysqli_query($conn, "SELECT COUNT(*) AS total FROM set_akte");
$totalRow    = mysqli_fetch_assoc($totalResult);
$totalData   = $totalRow['total'];
$totalPages  = ceil($totalData / $limit);

// ambil data sesuai halaman
$sql = "SELECT tahun, bulan, pendapatan_akte, persen_rs, bagian_rs, persen_kry, bagian_kry
        FROM set_akte
