...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['mode']) && $_POST['mode'] === 'update') {
        ...
        $stmt->bind_param("ii", $persen, $kode);
        $stmt->execute();
...
// ambil total_insentif dari tabel set_insentif
$stmt = $conn->prepare("SELECT total_insentif FROM set_insentif WHERE tahun=? AND bulan=?");
...
// hitung summary dari indexins (tanpa filter tahun-bulan)
$sumResult = mysqli_query($conn, "
    SELECT COUNT(*) AS jml_departemen,
           SUM(persen) AS ttl_persen
    FROM indexins
");
$sumRow = mysqli_fetch_assoc($sumResult);
$jmlDepartemen = $sumRow['jml_departemen'] ?? 0;
$ttlPersen     = $sumRow['ttl_persen'] ?? 0;

// total insentif departemen = (totalInsentif / 100) * ttlPersen
    ...
    <div class="card shadow">
      <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
        <h5 class="mb-0 text-uppercase flex-grow-1 text-center">
          DETAIL INSENTIF - <?= htmlspecialchars($tahun) . " " . $bulanArr[(int)$bulan] ?>
        </h5>
        <div class="d-flex gap-2">
          <button class="btn btn-light btn-sm" data-bs-toggle="modal" data-bs-target="#modalTambah">➕ Tambah Penerima</button>
          <a href="insentif.php" class="btn btn-secondary btn-sm">⬅️ Kembali</a>
        </div>
      </div>
      <div class="card-body p-3">
        <div class="table-wrapper">
          ...
          <div class="mt-2 small text-start text-muted">
            ...
            Ttl Insentif : Rp. <?= number_format($ttlInsentif, 0, ',', '.') ?>
          </div>
