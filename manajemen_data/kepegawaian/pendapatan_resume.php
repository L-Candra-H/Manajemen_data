...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $pendapatan = $_POST['pendapatan_resume'] ?? null;
    ...
    if ($_POST['mode'] === 'update') {
        $stmt = $conn->prepare("UPDATE set_resume SET pendapatan_resume=?, persen_rs=?, bagian_rs=?, persen_kry=?, bagian_kry=? WHERE tahun=? AND bulan=?");
        ...
        $cek = $conn->prepare("SELECT COUNT(*) FROM set_resume WHERE tahun=? AND bulan=?");
        ...
        if ($ada > 0) {
            ...
            $stmt = $conn->prepare("INSERT INTO set_resume (tahun, bulan, pendapatan_resume, persen_rs, bagian_rs, persen_kry, bagian_kry) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("iiiiiii", $tahun, $bulan, $pendapatan, $persen_rs, $bagian_rs, $persen_kry, $bagian_kry);
            $stmt->execute();
        }

    } elseif ($_POST['mode'] === 'delete') {
        $stmt = $conn->prepare("DELETE FROM set_resume WHERE tahun=? AND bulan=?");
...
// hitung total data
$totalResult = mysqli_query($conn, "SELECT COUNT(*) AS total FROM set_resume");
$totalRow    = mysqli_fetch_assoc($totalResult);
$totalData   = $totalRow['total'];
$totalPages  = ceil($totalData / $limit);

// ambil data sesuai halaman
$sql = "SELECT tahun, bulan, pendapatan_resume, persen_rs, bagian_rs, persen_kry, bagian_kry
        FROM set_resume
...
<main class="main-content container-fluid mt-4">
  <div class="card shadow">
    <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
      <h5 class="mb-0 text-uppercase flex-grow-1 text-center">Pendapatan Resume</h5>
        ...
        <table class="table table-striped table-bordered table-setting align-middle">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td><?= "Rp. " . number_format($row['pendapatan_resume'], 0, ',', '.') ?></td>
              ...
              <td class="text-center">
                <button class="btn btn-warning btn-sm"
                        ...
                        data-pendapatan="<?= htmlspecialchars($row['pendapatan_resume']) ?>"
                        data-persenrs="<?= htmlspecialchars($row['persen_rs']) ?>"
                        data-persenkry="<?= htmlspecialchars($row['persen_kry']) ?>">
                  ✏️ Edit
                </button>
                <a href="detail_pendapatan_resume.php?tahun=<?= $row['tahun'] ?>&bulan=<?= $row['bulan'] ?>" class="btn btn-info btn-sm me-2">💰 Detail Pembagian</a>
  ...
  <div class="modal-dialog">
    <form action="" method="post" class="modal-content">
      ...
      <div class="modal-header bg-success text-white">
        <h5 class="modal-title">Tambah Pendapatan Resume</h5>
      ...
      <div class="modal-body">
        ...
        <div class="mb-3">
          <label class="form-label">Pendapatan Resume</label>
          <input type="number" name="pendapatan_resume" class="form-control" required>
        </div>
        <div class="mb-3">
  ...
  <div class="modal-dialog">
    <form action="" method="post" class="modal-content">
      ...
      <div class="modal-header bg-warning">
        <h5 class="modal-title">Edit Pendapatan Resume</h5>
      ...
      <div class="modal-body">
        ...
        <div class="mb-3">
          <label class="form-label">Pendapatan Resume</label>
          <input type="number" name="pendapatan_resume" id="editPendapatan" class="form-control" required>
        </div>
        <div class="mb-3">
          <label class="form-label">% RS</label>
