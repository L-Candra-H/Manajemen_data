...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['mode']) && $_POST['mode'] === 'update') {
        $stmt = $conn->prepare("UPDATE pembagian_tuslah SET persen=? WHERE id=?");
        $stmt->bind_param("ss", $persen, $kode);
        $stmt->execute();
    } elseif (isset($_POST['mode']) && $_POST['mode'] === 'insert') {
        $stmt = $conn->prepare("INSERT INTO pembagian_tuslah (id, persen) VALUES (?, ?)");
        $stmt->bind_param("ss", $kode, $persen);
        $stmt->execute();
    } elseif (isset($_POST['mode']) && $_POST['mode'] === 'delete') {
        $stmt = $conn->prepare("DELETE FROM pembagian_tuslah WHERE id=?");
...
// ambil bagian_kry dari tabel set_tuslah
$stmt = $conn->prepare("SELECT bagian_kry FROM set_tuslah WHERE tahun=? AND bulan=?");
...
// hitung total data
$totalResult = mysqli_query($conn, "SELECT COUNT(*) AS total FROM pembagian_tuslah");
...
$sql  = "SELECT i.id, d.nik, d.nama, i.persen
         FROM pembagian_tuslah i
         JOIN pegawai d ON i.id = d.id
         ORDER BY i.id
         LIMIT $limit OFFSET $offset";
$result = mysqli_query($conn, $sql);

// daftar pegawai yang belum punya pembagian_tuslah (untuk dropdown tambah)
$listDep = $conn->query("
    SELECT d.id, d.nik, d.nama
    FROM pegawai d
    LEFT JOIN pembagian_tuslah i ON d.id = i.id
    WHERE i.id IS NULL
...
<head>
  <meta charset="UTF-8">
  <title>Detail Pembagian Tuslah</title>
    ...
    <div class="card shadow">
      <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
        <h5 class="mb-0 text-uppercase flex-grow-1 text-center">
          DETAIL PEMBAGIAN TUSLAH - <?= htmlspecialchars($tahun) . " " . $bulanArr[(int)$bulan] ?>
        </h5>
        <div class="d-flex gap-2">
          <button class="btn btn-light btn-sm" data-bs-toggle="modal" data-bs-target="#modalTambah">➕ Tambah Penerima</button>
          <a href="pendapatan_tuslah.php" class="btn btn-secondary btn-sm">⬅️ Kembali</a>
        </div>
      </div>
    ...
    <div class="modal-dialog">
      <form action="" method="post" class="modal-content">
        ...
        <div class="modal-header bg-success text-white">
          <h5 class="modal-title">Tambah Pembagian Tuslah</h5>
        ...
        <div class="modal-body">
          ...
          <div class="mb-3">
            <label class="form-label">Porsi Tuslah (%)</label>
    ...
    <div class="modal-dialog">
      <form action="" method="post" class="modal-content">
        ...
        <div class="modal-header bg-warning">
          <h5 class="modal-title">Edit Pembagian Tuslah</h5>
