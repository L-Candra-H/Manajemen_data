...
if ($tahun != '') {
    // hitung total baris untuk pagination (jumlah jenis cidera)
    $countSql = "SELECT COUNT(*) AS jml FROM k3rs_jenis_cidera";
    $total_rows = $conn->query($countSql)->fetch_assoc()['jml'];

    // query data cidera per bulan (LEFT JOIN agar semua jenis tampil)
    $sql = "SELECT c.jenis_cidera,
            ...
            FROM k3rs_jenis_cidera c
            LEFT JOIN k3rs_peristiwa p ON p.kode_cidera = c.kode_cidera
            GROUP BY c.jenis_cidera
            ORDER BY c.jenis_cidera ASC
            LIMIT ? OFFSET ?";
...
<head>
  <meta charset="UTF-8">
  <title>Jenis Cidera K3 Per Tahun</title>
    ...
    <div class="container-fluid mt-4">
      <div class="card shadow">
        ...
        <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
          <h5 class="mb-0 text-uppercase flex-grow-1 text-center">Jenis Cidera K3 Per Tahun</h5>
