...
if ($tahun != '') {
    // hitung total baris untuk pagination (jumlah lokasi kejadian)
    $countSql = "SELECT COUNT(*) AS jml FROM k3rs_lokasi_kejadian";
    $total_rows = $conn->query($countSql)->fetch_assoc()['jml'];

    // query data lokasi kejadian per bulan (LEFT JOIN agar semua jenis tampil)
    $sql = "SELECT c.lokasi_kejadian,
            ...
            FROM k3rs_lokasi_kejadian c
            LEFT JOIN k3rs_peristiwa p ON p.kode_lokasi = c.kode_lokasi
            GROUP BY c.lokasi_kejadian
            ORDER BY c.lokasi_kejadian ASC
            LIMIT ? OFFSET ?";
...
<head>
  <meta charset="UTF-8">
  <title>Lokasi Kejadian K3 per Tahun</title>
    ...
    <div class="container-fluid mt-4">
      <div class="card shadow">
        ...
        <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
          <h5 class="mb-0 text-uppercase flex-grow-1 text-center">Lokasi Kejadian K3 Per Tahun</h5>
