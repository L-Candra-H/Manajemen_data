...
$result = mysqli_query($conn, "SELECT * FROM k3rs_peristiwa ORDER BY kode_dampak ASC LIMIT $start, $limit");
...
if ($tahun != '') {
    // hitung total baris untuk pagination (jumlah dampak cidera)
    $countSql = "SELECT COUNT(*) AS jml FROM k3rs_dampak_cidera";
    $total_rows = $conn->query($countSql)->fetch_assoc()['jml'];

    // query data dampak cidera per bulan (LEFT JOIN agar semua jenis tampil)
    $sql = "SELECT c.dampak_cidera,
            ...
            FROM k3rs_dampak_cidera c
            LEFT JOIN k3rs_peristiwa p ON p.kode_dampak = c.kode_dampak
            GROUP BY c.dampak_cidera
            ORDER BY c.dampak_cidera ASC
            LIMIT ? OFFSET ?";
...
<head>
  <meta charset="UTF-8">
  <title>Dampak Cidera K3 per Tahun</title>
    ...
    <div class="container-fluid mt-4">
      <div class="card shadow">
        ...
        <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
          <h5 class="mb-0 text-uppercase flex-grow-1 text-center">Dampak Cidera K3 Per Tahun</h5>
