...
$result = mysqli_query($conn, "SELECT * FROM k3rs_peristiwa ORDER BY kode_pekerjaan ASC LIMIT $start, $limit");
...
if ($tahun != '') {
    // hitung total baris untuk pagination (jumlah jenis pekerjaan)
    $countSql = "SELECT COUNT(*) AS jml FROM k3rs_jenis_pekerjaan";
    $total_rows = $conn->query($countSql)->fetch_assoc()['jml'];

    // query data jenis pekerjaan per bulan (LEFT JOIN agar semua jenis tampil)
    $sql = "SELECT c.jenis_pekerjaan,
            ...
            FROM k3rs_jenis_pekerjaan c
            LEFT JOIN k3rs_peristiwa p ON p.kode_pekerjaan = c.kode_pekerjaan
            GROUP BY c.jenis_pekerjaan
            ORDER BY c.jenis_pekerjaan ASC
            LIMIT ? OFFSET ?";
...
<head>
  <meta charset="UTF-8">
  <title>Jenis Pekerjaan K3 per Tahun</title>
    ...
    <div class="container-fluid mt-4">
      <div class="card shadow">
        ...
        <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
          <h5 class="mb-0 text-uppercase flex-grow-1 text-center">Jenis Pekerjaan K3 Per Tahun</h5>
