...
$result = mysqli_query($conn, "SELECT * FROM k3rs_peristiwa ORDER BY kode_bagian ASC LIMIT $start, $limit");
...
if ($tahun != '') {
    // hitung total baris untuk pagination (jumlah bagian tubuh)
    $countSql = "SELECT COUNT(*) AS jml FROM k3rs_bagian_tubuh";
    $total_rows = $conn->query($countSql)->fetch_assoc()['jml'];

    // query data bagian tubuh per bulan (LEFT JOIN agar semua bagian tampil)
    $sql = "SELECT c.bagian_tubuh,
            ...
            FROM k3rs_bagian_tubuh c
            LEFT JOIN k3rs_peristiwa p ON p.kode_bagian = c.kode_bagian
            GROUP BY c.bagian_tubuh
            ORDER BY c.bagian_tubuh ASC
            LIMIT ? OFFSET ?";
...
<head>
  <meta charset="UTF-8">
  <title>Bagian Tubuh K3 per Tahun</title>
    ...
    <div class="container-fluid mt-4">
      <div class="card shadow">
        ...
        <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
          <h5 class="mb-0 text-uppercase flex-grow-1 text-center">Bagian Tubuh K3 Per Tahun</h5>
