...
$result = mysqli_query($conn, "SELECT * FROM k3rs_peristiwa ORDER BY kode_penyebab ASC LIMIT $start, $limit");
...
if ($tahun != '') {
    // hitung total baris untuk pagination (jumlah penyebab kecelakaan)
    $countSql = "SELECT COUNT(*) AS jml FROM k3rs_penyebab";
    $total_rows = $conn->query($countSql)->fetch_assoc()['jml'];

    // query data penyebab kecelakaan per bulan (LEFT JOIN agar semua jenis tampil)
    $sql = "SELECT c.penyebab_kecelakaan,
            ...
            FROM k3rs_penyebab c
            LEFT JOIN k3rs_peristiwa p ON p.kode_penyebab = c.kode_penyebab
            GROUP BY c.penyebab_kecelakaan
            ORDER BY c.penyebab_kecelakaan ASC
            LIMIT ? OFFSET ?";
...
<head>
  <meta charset="UTF-8">
  <title>Penyebab Kecelakaan K3 per Tahun</title>
    ...
    <div class="container-fluid mt-4">
      <div class="card shadow">
        ...
        <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
          <h5 class="mb-0 text-uppercase flex-grow-1 text-center">Penyebab Kecelakaan K3 Per Tahun</h5>
