...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['aksi'])) {
    ...
    if ($aksi === 'tambah') {
        ...
        $audit1 = $_POST['pemasangan_sesuai_indikasi'];
        $audit2 = $_POST['hand_hygiene'];
        $audit3 = $_POST['menggunakan_apd_yang_tepat'];
        $audit4 = $_POST['pemasangan_menggunakan_alat_steril'];
        $audit5 = $_POST['segera_dilepas_setelah_tidak_diperlukan'];
        $audit6 = $_POST['pengisian_balon_sesuai_petunjuk'];
        $audit7 = $_POST['fiksasi_kateter_dengan_plester'];
        $audit8 = $_POST['urinebag_menggantung_tidak_menyentuh_lantai'];

        $sql = "INSERT INTO audit_bundle_isk
                (tanggal, id_ruang, pemasangan_sesuai_indikasi, hand_hygiene, menggunakan_apd_yang_tepat, pemasangan_menggunakan_alat_steril,
                segera_dilepas_setelah_tidak_diperlukan, pengisian_balon_sesuai_petunjuk, fiksasi_kateter_dengan_plester, urinebag_menggantung_tidak_menyentuh_lantai)
                VALUES (?,?,?,?,?,?,?,?,?,?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssssssssss", $tanggal, $id_ruang,$audit1,$audit2,$audit3,$audit4,$audit5,$audit6,$audit7,$audit8);
        $stmt->execute();
    }
    ...
    if ($aksi === 'update') {
        ...
        $sql = "UPDATE audit_bundle_isk SET
                    tanggal=?, id_ruang=?, 
                    pemasangan_sesuai_indikasi=?, hand_hygiene=?, menggunakan_apd_yang_tepat=?, pemasangan_menggunakan_alat_steril=?,
                    segera_dilepas_setelah_tidak_diperlukan=?, pengisian_balon_sesuai_petunjuk=?, fiksasi_kateter_dengan_plester=?, urinebag_menggantung_tidak_menyentuh_lantai=?
                WHERE id_ruang=? AND tanggal=?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssssssssssss", $tanggal, $id_ruang,$audit1,$audit2,$audit3,$audit4,$audit5,$audit6,$audit7,$audit8,
    ...
    if ($aksi === 'hapus') {
        ...
        $sql = "DELETE FROM audit_bundle_isk WHERE id_ruang=? AND tanggal=?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ss", $id_ruang, $tanggal);
        $stmt->execute();

        header("Location: audit_bundle_isk.php");
        exit;
    }
}
...
// === HITUNG TOTAL DATA ===
$sqlCount = "SELECT COUNT(*) AS total FROM audit_bundle_isk ".($awalForm&&$akhirForm?"WHERE tanggal BETWEEN ? AND ?":"WHERE 1=0");
...
$sqlRekap = "SELECT
    SUM(pemasangan_sesuai_indikasi='Ya') AS a1_ya, SUM(pemasangan_sesuai_indikasi='Tidak') AS a1_tidak,
    SUM(hand_hygiene='Ya') AS a2_ya, SUM(hand_hygiene='Tidak') AS a2_tidak,
    SUM(menggunakan_apd_yang_tepat='Ya') AS a3_ya, SUM(menggunakan_apd_yang_tepat='Tidak') AS a3_tidak,
    SUM(pemasangan_menggunakan_alat_steril='Ya') AS a4_ya, SUM(pemasangan_menggunakan_alat_steril='Tidak') AS a4_tidak,
    SUM(segera_dilepas_setelah_tidak_diperlukan='Ya') AS a5_ya, SUM(segera_dilepas_setelah_tidak_diperlukan='Tidak') AS a5_tidak,
    SUM(pengisian_balon_sesuai_petunjuk='Ya') AS a6_ya, SUM(pengisian_balon_sesuai_petunjuk='Tidak') AS a6_tidak,
    SUM(fiksasi_kateter_dengan_plester='Ya') AS a7_ya, SUM(fiksasi_kateter_dengan_plester='Tidak') AS a7_tidak,
    SUM(urinebag_menggantung_tidak_menyentuh_lantai='Ya') AS a8_ya, SUM(urinebag_menggantung_tidak_menyentuh_lantai='Tidak') AS a8_tidak,
    COUNT(*) AS total
    FROM audit_bundle_isk 
...
// === HITUNG TTL ===
$totalYa =$rekap['a1_ya']+$rekap['a2_ya']+$rekap['a3_ya']+$rekap['a4_ya']+$rekap['a5_ya']+$rekap['a6_ya']+$rekap['a7_ya']+$rekap['a8_ya'];
$totalTidak =$rekap['a1_tidak']+$rekap['a2_tidak']+$rekap['a3_tidak']+$rekap['a4_tidak']+$rekap['a5_tidak']+$rekap['a6_tidak']+$rekap['a7_tidak']+$rekap['a8_tidak'];
    ...
    <div class="container-fluid mt-4">
      <div class="card shadow">
        ...
        <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
          <h5 class="mb-0 text-uppercase flex-grow-1 text-center">Audit Bundle ISK</h5>
            ...
            <table class="table table-striped table-bordered table-master_audit align-middle">
              <thead class="table-dark text-center">
                <tr>
                  ...
                  <th>ID Ruang</th>
                  <th>Ruang/Unit</th>
                  <th>1. Pemasangan Sesuai Indikasi</th>
                  <th>2. Hand Hygiene</th>
                  <th>3. Menggunakan APD Yang Tepat</th>
                  <th>4. Pemasangan Menggunakan Alat Steril</th>
                  <th>5. Segera Dilepas Setelah Tidak Diperlukan</th>
                  <th>6. Pengisian Balon Sesuai Petunjuk Produk (20 ml)</th>
                  <th>7. Fiksasi Kateter Dengan Plester</th>
                  <th>8. Urine Bag Menggantung Tidak Menyentuh Tanah</th>
                  <th>Ttl. Nilai (%)</th>
                  <th>Aksi</th>
                </tr>
              </thead>
              <tbody>
                ...
                <?php else: ?>
                  ...
                  <tr>
                    ...
                    <td><?=$row['pemasangan_sesuai_indikasi']?></td>
                    <td><?=$row['hand_hygiene']?></td>
                    <td><?=$row['menggunakan_apd_yang_tepat']?></td>
                    <td><?=$row['pemasangan_menggunakan_alat_steril']?></td>
                    <td><?=$row['segera_dilepas_setelah_tidak_diperlukan']?></td>
                    <td><?=$row['pengisian_balon_sesuai_petunjuk']?></td>
                    <td><?=$row['fiksasi_kateter_dengan_plester']?></td>
                    <td><?=$row['urinebag_menggantung_tidak_menyentuh_lantai']?></td>
                    <td><?=$ttlNilai?>%</td>
                    <td class="text-center">
                      ...
                      <button type="button" class="btn btn-warning btn-sm"
                        ...
                        data-audit1="<?=$row['pemasangan_sesuai_indikasi']?>"
                        data-audit2="<?=$row['hand_hygiene']?>"
                        data-audit3="<?=$row['menggunakan_apd_yang_tepat']?>"
                        data-audit4="<?=$row['pemasangan_menggunakan_alat_steril']?>"
                        data-audit5="<?=$row['segera_dilepas_setelah_tidak_diperlukan']?>"
                        data-audit6="<?=$row['pengisian_balon_sesuai_petunjuk']?>"
                        data-audit7="<?=$row['fiksasi_kateter_dengan_plester']?>"
                        data-audit8="<?=$row['urinebag_menggantung_tidak_menyentuh_lantai']?>">
                        ✏️ Edit
                      </button>
                      <!-- Tombol Hapus -->
      ...
      <div class="modal-content">
        <form method="post" action="">
          ...
          <div class="modal-header bg-success text-white">
            <h5 class="modal-title">Tambah Audit Bundle ISK</h5>
          ...
          <div class="modal-body">
            ...
            <div class="row g-3 mb-2">
              <div class="col-md-6 d-flex align-items-center gap-3">
                <label class="flex-grow-1">1. Pemasangan Sesuai Indikasi</label>
                <select name="pemasangan_sesuai_indikasi" class="form-select w-auto">
                  <option value="Ya">Ya</option>
                  <option value="Tidak">Tidak</option>
                </select>
              </div>
              <div class="col-md-6 d-flex align-items-center gap-3">
                <label class="flex-grow-1">2. Hand Hygiene</label>
                <select name="hand_hygiene" class="form-select w-auto">
                  <option value="Ya">Ya</option>
                  <option value="Tidak">Tidak</option>
                </select>
              </div>
            ...
            <div class="row g-3 mb-2">
              <div class="col-md-6 d-flex align-items-center gap-3">
                <label class="flex-grow-1">3. Menggunakan APD Yang Tepat</label>
                <select name="menggunakan_apd_yang_tepat" class="form-select w-auto">
                  <option value="Ya">Ya</option>
                  <option value="Tidak">Tidak</option>
                </select>
              </div>
              <div class="col-md-6 d-flex align-items-center gap-3">
                <label class="flex-grow-1">4. Pemasangan Menggunakan Alat Steril</label>
                <select name="pemasangan_menggunakan_alat_steril" class="form-select w-auto">
                  <option value="Ya">Ya</option>
                  <option value="Tidak">Tidak</option>
                </select>
              </div>
            ...
            <div class="row g-3 mb-2">
              <div class="col-md-6 d-flex align-items-center gap-3">
                <label class="flex-grow-1">5. Segera Dilepas Setelah Tidak Diperlukan</label>
                <select name="segera_dilepas_setelah_tidak_diperlukan" class="form-select w-auto">
                  <option value="Ya">Ya</option>
                  <option value="Tidak">Tidak</option>
                </select>
              </div>
              <div class="col-md-6 d-flex align-items-center gap-3">
                <label class="flex-grow-1">6. Pengisian Balon Sesuai Petunjuk Produk (20 ml)</label>
                <select name="pengisian_balon_sesuai_petunjuk" class="form-select w-auto">
                  <option value="Ya">Ya</option>
                  <option value="Tidak">Tidak</option>
                </select>
              </div>
            ...
            <div class="row g-3 mb-2">
              <div class="col-md-6 d-flex align-items-center gap-3">
               <label class="flex-grow-1">7. Fiksasi Kateter Dengan Plester</label>
                <select name="fiksasi_kateter_dengan_plester" class="form-select w-auto">
                  <option value="Ya">Ya</option>
                  <option value="Tidak">Tidak</option>
                </select>
              </div>
              <div class="col-md-6 d-flex align-items-center gap-3">
                <label class="flex-grow-1">8. Urine Bag Menggantung Tidak Menyentuh Tanah</label>
                <select name="urinebag_menggantung_tidak_menyentuh_lantai" class="form-select w-auto">
                  <option value="Ya">Ya</option>
                  <option value="Tidak">Tidak</option>
                </select>
              </div>
        ...
        <form method="post" action="">
          ...
          <div class="modal-body">
            ...
            <div class="row g-3 mb-2">
              <div class="col-md-6 d-flex align-items-center gap-3">
                <label class="flex-grow-1">1. Pemasangan Sesuai Indikasi</label>
                <select name="pemasangan_sesuai_indikasi" id="edit_audit1" class="form-select w-auto">
                  <option value="Ya">Ya</option>
                  <option value="Tidak">Tidak</option>
                </select>
              </div>
              <div class="col-md-6 d-flex align-items-center gap-3">
                <label class="flex-grow-1">2. Hand Hygiene</label>
                <select name="hand_hygiene" id="edit_audit2" class="form-select w-auto">
                  <option value="Ya">Ya</option>
                  <option value="Tidak">Tidak</option>
                </select>
              </div>
            ...
            <div class="row g-3 mb-2">
              <div class="col-md-6 d-flex align-items-center gap-3">
                <label class="flex-grow-1">3. Menggunakan APD Yang Tepat</label>
                <select name="menggunakan_apd_yang_tepat" id="edit_audit3" class="form-select w-auto">
                  <option value="Ya">Ya</option>
                  <option value="Tidak">Tidak</option>
                </select>
              </div>
              <div class="col-md-6 d-flex align-items-center gap-3">
                <label class="flex-grow-1">4. Pemasangan Menggunakan Alat Steril</label>
                <select name="pemasangan_menggunakan_alat_steril" id="edit_audit4" class="form-select w-auto">
                  <option value="Ya">Ya</option>
                  <option value="Tidak">Tidak</option>
                </select>
              </div>
            ...
            <div class="row mb-2">
              <div class="col-md-6 d-flex align-items-center gap-3">
                <label class="flex-grow-1">5. Segera Dilepas Setelah Tidak Diperlukan</label>
                <select name="segera_dilepas_setelah_tidak_diperlukan" id="edit_audit5" class="form-select w-auto">
                  <option value="Ya">Ya</option>
                  <option value="Tidak">Tidak</option>
                </select>
              </div>
              <div class="col-md-6 d-flex align-items-center gap-3">
                <label class="flex-grow-1">6. Pengisian Balon Sesuai Petunjuk Produk (20 ml)</label>
                <select name="pengisian_balon_sesuai_petunjuk" id="edit_audit6" class="form-select w-auto">
                  <option value="Ya">Ya</option>
                  <option value="Tidak">Tidak</option>
                </select>
              </div>
            ...
            <div class="row g-3 mb-2">
              <div class="col-md-6 d-flex align-items-center gap-3">
                <label class="flex-grow-1">7. Fiksasi Kateter Dengan Plester</label>
                <select name="fiksasi_kateter_dengan_plester" id="edit_audit7" class="form-select w-auto">
                  <option>Ya</option>
                  <option>Tidak</option>
                </select>
              </div>
              <div class="col-md-6 d-flex align-items-center gap-3">
                <label class="flex-grow-1">8. Urine Bag Menggantung Tidak Menyentuh Tanah</label>
                <select name="urinebag_menggantung_tidak_menyentuh_lantai" id="edit_audit8" class="form-select w-auto">
                  <option>Ya</option>
                  <option>Tidak</option>
                </select>
              </div>
